...
export function registerCustomHelpers() {
  CustomHandlebars.registerHelper('sql', function (stmt, options) {
    ...
    try {
      ...
      return "BAD <br/>" + err.message;
    }
  });

  CustomHandlebars.registerHelper('local', function (context, options) {
    try {
      let addon:any = {};
      for(let k in options.hash) {
        addon[k] = CustomHandlebars.compile(options.hash[k])(context);
      }
      let new_context = Object.assign(context, addon);
...
export async function mw_render_html(ctx: ParameterizedContext, next: Next) {
  ...
  if (no_redirect && (is_html || is_handlebars) && ctx.response.body) {
